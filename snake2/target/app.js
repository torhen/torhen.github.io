// Transcrypt'ed from Python, 2022-01-14 22:40:13
var random = {};
var time = {};
...
export var Snake =  __class__ ('Snake', [object], {
	...
	get reset () {return __get__ (this, function (self) {
		...
		for (var i = 1; i < initial_block_len; i++) {
	...
	get set_dir () {return __get__ (this, function (self, dx, dy) {
		var x0 = self.blocks [0].x + dx;
		var y0 = self.blocks [0].y + dy;
		var x1 = self.blocks [1].x;
		var y1 = self.blocks [1].y;
		if (x0 == x1 && y0 == y1) {
			return ;
		}
		self.dx = dx;
	...
	get is_on_snake () {return __get__ (this, function (self, x, y) {
	...
	get place_apple () {return __get__ (this, function (self) {
		while (true) {
			var x = int (random.random () * g_width);
			var y = int (random.random () * g_height);
			if (!(self.is_on_snake (x, y))) {
	...
	get check_self_coll () {return __get__ (this, function (self) {
		var x0 = self.blocks [0].x;
		var y0 = self.blocks [0].y;
		for (var block of self.blocks.__getslice__ (1, null, 1)) {
			var x1 = block.x;
			var y1 = block.y;
			if (x0 == x1 && y0 == y1) {
				var s = 'You achieved {} blocks.'.format (len (self.blocks));
				alert (s);
				self.reset ();
			}
		}
...
export var myButton = document.getElementById ('button_up');
myButton.style.width = '600px';
myButton.style.height = '100px';
var myButton = document.getElementById ('button_left');
myButton.style.width = '300px';
myButton.style.height = '100px';
var myButton = document.getElementById ('button_right');
myButton.style.width = '300px';
myButton.style.height = '100px';
